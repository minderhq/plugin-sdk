"""Weather — a complete, self-contained reference plugin.

Shows every part of the contract a real plugin uses:
  * the lifecycle (register / initialize / health_check / collect_data /
    analyze / shutdown),
  * CONFIG_SCHEMA + apply_config (runtime-editable config, no restart),
  * ACTIONS + READ_ONLY_ACTIONS (HTTP-invokable methods),
  * AI_TOOLS (an Ollama / OpenAI function-calling tool).

It polls a public, keyless weather API (Open-Meteo) for the configured locations
and can sink the readings to InfluxDB (the storage config is injected by the
registry as ``config["influxdb"]``). Depends only on ``httpx`` + the SDK.
"""
...
class WeatherPlugin:
        ...
        for item in spec.split(","):
            ...
            if len(parts) == 3:
                try:
                    ...
                    logger.warning(f"bad WEATHER_LOCATIONS entry: {item!r}")
        return out
    ...
    async def _geocode(self, name: str) -> Optional[Tuple[float, float]]:
        ...
        try:
            ...
            logger.warning(f"geocode failed: {type(e).__name__}: {e}")
            return None
    ...
    async def collect_data(self) -> Dict:
        ...
        logger.info(f"weather collect: {len(readings)} location(s), influx={wrote}")
        return self._last
